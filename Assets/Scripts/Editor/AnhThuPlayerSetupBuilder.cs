...
public static class AnhThuPlayerSetupBuilder
{
    private const string BaseModelPath = "Assets/Models/Player/Action_Anh_Thu/Action_Anh_Thu/Anh_Thu@Model.fbx";
    ...
    private const string ControllerPath = "Assets/Animations/Player/AnhThu.controller";
    private const string MaterialFolder = "Assets/Models/Player/Materials";
    private const string AnhThuMaterialPath = MaterialFolder + "/AnhThu_Player_Color.mat";
    private const string BaseColorTexturePath = "Assets/Models/Player/texture_pbr_20250901 1.png";
    private const string NormalTexturePath = "Assets/Models/Player/texture_pbr_20250901_normal 1.png";
    ...
    [MenuItem("Tools/Dong Chay Anh Hung/Setup Anh Thu Player")]
    public static void SetupAnhThuPlayer()
    {
    ...
    private static Avatar EnsureBaseAvatar()
    {
        ...
        if (importer == null)
        {
            Debug.LogError("AnhThuPlayerSetupBuilder: missing Anh Thu model at " + BaseModelPath);
            return null;
        }
        ...
        if (avatar == null)
            Debug.LogWarning("AnhThuPlayerSetupBuilder: no avatar found in Anh Thu model.");
        else if (!avatar.isHuman || !avatar.isValid)
            Debug.LogWarning("AnhThuPlayerSetupBuilder: Anh Thu avatar is not a valid Humanoid avatar.");
    ...
    private static void ConfigureActionImporter(string path, Avatar avatar, string clipName, bool loop, bool copyAvatar)
    {
        ...
        if (importer == null)
        {
            Debug.LogWarning("AnhThuPlayerSetupBuilder: missing FBX at " + path);
            return;
        }
    ...
    private static int ReplaceScenePlayers(AnimatorController controller, Avatar avatar, Material playerMaterial)
    {
        ...
        if (model == null)
        {
            Debug.LogError("AnhThuPlayerSetupBuilder: model missing at " + BaseModelPath);
            return 0;
        }
    ...
    private static GameObject[] FindPlayerObjects()
    {
        ...
        foreach (PlayerController3D controller in Object.FindObjectsByType<PlayerController3D>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            if (controller != null)
                players.Add(controller.gameObject);
        }

        foreach (PlayerHealth3D health in Object.FindObjectsByType<PlayerHealth3D>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            if (health != null)
    ...
    private static Material GetOrCreateAnhThuMaterial()
    {
        EnsureFolder(MaterialFolder);

        Material material = AssetDatabase.LoadAssetAtPath<Material>(AnhThuMaterialPath);
        if (material == null)
        {
            ...
            {
                name = "AnhThu_Player_Color"
            };
            AssetDatabase.CreateAsset(material, AnhThuMaterialPath);
        }
    ...
    {
        string reportPath = Path.Combine(Directory.GetCurrentDirectory(), "Library", "CodexBridge", "anh_thu_player_setup_report.json");
