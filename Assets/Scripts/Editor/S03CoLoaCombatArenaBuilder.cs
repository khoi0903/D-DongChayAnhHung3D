...
public static class S03CoLoaCombatArenaBuilder
{
    private const string RootName = "S03_CoLoaCombatIntegration";
    private const string OldGrayboxRootName = "S03_SinglePlayerArena_Generated";
    ...
    [MenuItem("Tools/Dong Chay Anh Hung/Rebuild S03 Co Loa Combat Map")]
    public static void BuildScene()
    {
        ...
        GameObject map = EnsureCoLoaMap();
        Bounds mapBounds = CalculateRenderableBounds(map);
        GameObject root = new GameObject(RootName);

        Material floorMat = CreateMaterial("S03_CoLoa_FloorGuide_Mat", new Color32(62, 66, 74, 255), 0.2f);
        Material wallMat = CreateMaterial("S03_CoLoa_Boundary_Mat", new Color32(38, 42, 48, 255), 0.16f);
        Material accentMat = CreateMaterial("S03_CoLoa_Bronze_Mat", new Color32(184, 122, 40, 255), 0.35f);
        Material blueMat = CreateMaterial("S03_CoLoa_BlueGlow_Mat", new Color32(40, 180, 255, 255), 0.55f, new Color(0.05f, 0.7f, 1.3f));
        Material redMat = CreateMaterial("S03_CoLoa_RedGlow_Mat", new Color32(210, 48, 60, 255), 0.55f, new Color(1.2f, 0.08f, 0.08f));

        CombatLayout layout = BuildArena(root, mapBounds, floorMat, wallMat, accentMat, blueMat, redMat);
        SetupLighting();
        ...
        director.Configure(
            ...
            waveText,
            statusText);
        director.ConfigureWaveTuning(3, 1, 12, 0, layout.Radius, 1.2f, 1.15f, 8f, 12, 0f, true);

        Selection.activeGameObject = root;
        EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene(), ScenePath);
        Debug.Log("S03 Co Loa combat map rebuilt with player, Hac Tinh waves, health UI, and Blessings.");
    }

    [MenuItem("Tools/Dong Chay Anh Hung/Verify S03 Co Loa Combat Map")]
    ...
    private static GameObject EnsureCoLoaMap()
    {
        ...
        if (mapAsset == null)
            throw new UnityException("S03 Co Loa builder could not find map asset: " + CoLoaMapAssetPath);
    ...
    private static Bounds CalculateRenderableBounds(GameObject root)
    {
        ...
        if (renderers.Length == 0)
            return new Bounds(root != null ? root.transform.position : Vector3.zero, new Vector3(80f, 12f, 80f));

        Bounds bounds = renderers[0].bounds;
        for (int i = 1; i < renderers.Length; i++)
            bounds.Encapsulate(renderers[i].bounds);

        if (bounds.size.x < 4f || bounds.size.z < 4f)
            bounds.Expand(new Vector3(80f, 0f, 80f));

        return bounds;
    }

    private static CombatLayout BuildArena(GameObject root, Bounds mapBounds, Material floorMat, Material wallMat, Material accentMat, Material blueMat, Material redMat)
    {
        float minExtent = Mathf.Min(mapBounds.extents.x, mapBounds.extents.z);
        if (minExtent < 8f)
            minExtent = 32f;

        float radius = Mathf.Clamp(minExtent * 0.42f, 20f, 58f);
        float groundY = mapBounds.min.y + 0.08f;
        Vector3 center = new Vector3(mapBounds.center.x, groundY, mapBounds.center.z);

        CreateInvisibleColliderCube(root, "S03_CoLoa_CombatFloor", center + new Vector3(0f, -0.12f, 0f), new Vector3(radius * 2.25f, 0.24f, radius * 2.25f));
        CreateInvisibleColliderCube(root, "S03_CoLoa_Boundary_North", center + new Vector3(0f, 1.9f, radius + 1.4f), new Vector3(radius * 2.35f, 3.8f, 1.1f));
        CreateInvisibleColliderCube(root, "S03_CoLoa_Boundary_South", center + new Vector3(0f, 1.9f, -radius - 1.4f), new Vector3(radius * 2.35f, 3.8f, 1.1f));
        CreateInvisibleColliderCube(root, "S03_CoLoa_Boundary_East", center + new Vector3(radius + 1.4f, 1.9f, 0f), new Vector3(1.1f, 3.8f, radius * 2.35f));
        CreateInvisibleColliderCube(root, "S03_CoLoa_Boundary_West", center + new Vector3(-radius - 1.4f, 1.9f, 0f), new Vector3(1.1f, 3.8f, radius * 2.35f));

        float sealSize = Mathf.Min(radius * 0.55f, 18f);
        GameObject centerSeal = CreatePrimitive(root, "S03_CoLoa_CombatSeal", PrimitiveType.Cylinder, center + Vector3.up * 0.02f, Vector3.zero, new Vector3(sealSize, 0.035f, sealSize), accentMat);
        RemoveCollider(centerSeal);

        for (int i = 0; i < 16; i++)
        {
            float angle = i * 22.5f;
            float runeRadius = radius * (i % 2 == 0 ? 0.58f : 0.82f);
            Vector3 position = center + new Vector3(Mathf.Sin(angle * Mathf.Deg2Rad) * runeRadius, 0.08f, Mathf.Cos(angle * Mathf.Deg2Rad) * runeRadius);
            GameObject rune = CreateCube(root, "S03_CoLoa_PathRune_" + i.ToString("00"), position, new Vector3(0f, angle, 0f), new Vector3(0.38f, 0.08f, Mathf.Clamp(radius * 0.12f, 1.8f, 4.8f)), i % 2 == 0 ? blueMat : accentMat);
            RemoveCollider(rune);
        }
        ...
        {
            float angle = i * 45f;
            Vector3 position = center + new Vector3(Mathf.Sin(angle * Mathf.Deg2Rad) * radius * 0.82f, 0.55f, Mathf.Cos(angle * Mathf.Deg2Rad) * radius * 0.82f);
            GameObject point = new GameObject("S03_EnemySpawn_" + (i + 1).ToString("00"));
            point.transform.SetParent(root.transform, false);
            point.transform.position = position;
            spawnPoints[i] = point.transform;

            GameObject visual = CreatePrimitive(root, "S03_EnemySpawnMarker_" + (i + 1).ToString("00"), PrimitiveType.Cylinder, position + Vector3.down * 0.48f, Vector3.zero, new Vector3(1.25f, 0.04f, 1.25f), redMat);
        ...
        return new CombatLayout
        {
            Center = center,
            Radius = radius,
            PlayerSpawn = center + Vector3.up * 1.05f,
    ...
    private static void DeleteOldGeneratedObjects()
    {
        ...
        DeleteAllSceneObjectsWithPrefix("S03_CoLoa_");
