...
/// <summary>
/// PlayerCombat3D.cs
/// Quản lý tấn công của người chơi (Light Attack / Heavy Attack).
/// Đã được nâng cấp để tích hợp với hệ thống Blessing S03 (BlessingRuntimeController).
/// Gắn trên prefab Player.
/// </summary>
public class PlayerCombat3D : MonoBehaviour
{
        ...
        string debugLabel)
    {
        ...
        foreach (GameObject enemy in enemies)
        {
            ...
            {
                if (blessingRuntime != null) blessingRuntime.OnEnemyKilled(enemyHealth.transform.position);
                hitCount++;
                continue;
            }

            if (blessingRuntime != null) blessingRuntime.OnEnemyHit(enemyHealth, enemyHealth.transform.position, attackContext);
        ...
        if (blessingRuntime != null) blessingRuntime.AfterPlayerAttack(attackContext, attackDirection, hitEnemies);
