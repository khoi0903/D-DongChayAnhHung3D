...
/// <summary>
/// BlessingRuntimeController.cs
/// Component gắn trên Player, xử lý toàn bộ hiệu ứng runtime của Blessing Anh Linh:
///   - Thống kê passive (giáp, tốc độ, tốc đánh)
///   - Hiệu ứng Dash (barrier, decoy, dash damage, war elephant)
///   - Hiệu ứng combat (chí mạng, sét, post-dash, Nỏ Thần)
///   - Hiệu ứng kill (Khởi Nghĩa Mê Linh)
///   - Hệ thống Thành Cổ Loa (định kỳ tạo lá chắn)
///   - Xuân Kỷ Dậu (cuồng chiến)
/// </summary>

// ══════════════════════════════════════════════════════════════════
//  Struct: ngữ cảnh của một đòn đánh — tính toán 1 lần, dùng nhiều nơi
// ══════════════════════════════════════════════════════════════════
public struct BlessingAttackContext
{
    ...
    /// <summary>Tạo context không có blessing nào ảnh hưởng.</summary>
...
{
    // ── References ───────────────────────────────────────────────────
    [Header("References – tự resolve nếu để trống")]
    ...
    // ── Feedback Colors ──────────────────────────────────────────────
    [Header("Màu hiệu ứng theo nhánh anh hùng")]
    [SerializeField] private Color defenseColor    = new Color(0.1f,  0.72f, 1f,    0.55f); // An Dương Vương – xanh dương
    [SerializeField] private Color offenseColor    = new Color(1f,    0.58f, 0.08f, 0.62f); // Quang Trung    – cam
    [SerializeField] private Color mobilityColor   = new Color(0.25f, 1f,    0.62f, 0.58f); // Trưng Nhị      – xanh lá
    [SerializeField] private Color leadershipColor = new Color(0.95f, 0.18f, 0.32f, 0.58f); // Trưng Trắc     – đỏ
    ...
    private void Update()
    {
        ...
        // Detect dash start / end
        if (isDashing && !wasDashing) BeginDashBlessings();
        if (isDashing)                TickDashDamage();
        if (!isDashing && wasDashing) EndDashBlessings();
    ...
    // ──────────────────────────────────────────────────────────────────
    /// <summary>
    /// Gọi từ S03CoLoaArenaBuilder hoặc bất kỳ script nào cần inject references.
    /// </summary>
    ...
    // ──────────────────────────────────────────────────────────────────
    //  Choice Mode (tạm dừng input khi chọn Blessing)
    // ──────────────────────────────────────────────────────────────────
    public void SetChoiceMode(bool enabled)
    {
        ...
        // Hiện/ẩn cursor khi chọn Blessing
    ...
    //  Apply Blessing
    // ──────────────────────────────────────────────────────────────────
    /// <summary>Gọi từ BlessingManager sau khi người chơi chọn 1 Blessing.</summary>
    public void ApplyBlessing(BlessingDefinition blessing, int newStack)
    {
        ...
        // Xuân Kỷ Dậu – kích hoạt ngay khi nhận
        if (blessing.EffectType == BlessingEffectType.KyDauFrenzy)
            ActivateFrenzy(7.5f);

        // Thành Cổ Loa – tạo shield ngay
        ...
        // Feedback pulse màu theo nhánh anh hùng
    ...
    //  Stack Query API
    // ──────────────────────────────────────────────────────────────────
    /// <summary>Tổng số stack của một loại hiệu ứng (cộng qua tất cả blessing cùng EffectType).</summary>
    public int GetStack(BlessingEffectType effectType)
    {
        int total = 0;
        foreach (KeyValuePair<string, int> pair in blessingStacks)
        {
            if (blessingsById.TryGetValue(pair.Key, out BlessingDefinition b) &&
                b != null && b.EffectType == effectType)
                total += pair.Value;
        }
        return total;
    }

    public bool HasEffect(BlessingEffectType effectType) => GetStack(effectType) > 0;

    // ──────────────────────────────────────────────────────────────────
    //  Wave Events
    // ──────────────────────────────────────────────────────────────────
    /// <summary>Gọi từ S03ArenaDirector mỗi đầu wave.</summary>
    ...
    /// <summary>Trả về độ trễ spawn (giây) do hiệu ứng Cảnh Giới.</summary>
    ...
    /// <summary>Trả về bán kính tăng thêm của tầm phát hiện địch.</summary>
    public float GetEnemyAwarenessRangeBonus() => GetStack(BlessingEffectType.Awareness) * 2.5f;

    // ──────────────────────────────────────────────────────────────────
    //  Combat Event API (gọi từ PlayerCombat3D)
    // ──────────────────────────────────────────────────────────────────
    /// <summary>Tạo context đòn đánh có tính hết mọi blessing ảnh hưởng.</summary>
    public BlessingAttackContext CreateAttackContext(int baseDamage, Vector3 attackDirection)
    {
        ...
        if (IsFrenzyActive) damage *= 1.25f;

        // Post-dash bonus (Truy Kích)
        if (postDashReady)
        {
            int postStack = GetStack(BlessingEffectType.PostDashDamage);
            damage    *= 1f + postStack * 0.35f;
            postDashReady = false;
        }

        // Chí mạng (Đống Đa + Thiên Lôi Tây Sơn)
        int critStack      = GetStack(BlessingEffectType.CriticalPower);
        int lightningStack = GetStack(BlessingEffectType.CriticalLightning);
        float critChance   = 0.05f + critStack * 0.045f + lightningStack * 0.03f;
        bool isCritical    = Random.value < critChance;
        if (isCritical) damage *= 1.5f + critStack * 0.28f;

        // Nỏ Thần (mỗi 5 đòn)
        int crossbowStack  = GetStack(BlessingEffectType.DivineCrossbow);
        bool triggerNoCrossbow = crossbowStack > 0 && attackCounter % 5 == 0;

        // Ghi vào context
        context.Damage                = Mathf.Max(1, Mathf.RoundToInt(damage));
        context.IsCritical            = isCritical;
        context.TriggerDivineCrossbow = triggerNoCrossbow;
        context.ExtraProjectiles      = triggerNoCrossbow ? 3 : 0;
    ...
    /// <summary>Gọi mỗi khi đòn đánh trúng địch (xét sét, v.v.).</summary>
    ...
    /// <summary>Gọi mỗi khi địch bị hạ (hồi năng lượng, Dash, máu, v.v.).</summary>
    ...
    {
        if (playerController == null) playerController = GetComponent<PlayerController3D>();
        if (playerCombat     == null) playerCombat     = GetComponent<PlayerCombat3D>();
        if (playerHealth     == null) playerHealth     = GetComponent<PlayerHealth3D>();
    }

    // ──────────────────────────────────────────────────────────────────
    //  Private – Passive Stats Calculation
    // ──────────────────────────────────────────────────────────────────
    ...
    {
        // Tốc độ di chuyển (Kỵ Tướng)
        if (playerController != null)
        {
            float moveMultiplier = 1f + GetStack(BlessingEffectType.MoveSpeed) * 0.08f;
            if (IsFrenzyActive) moveMultiplier += 0.12f;
            playerController.moveSpeed = Mathf.Max(2f, baseMoveSpeed * moveMultiplier);

            // Hồi Dash (Thần Tốc Bắc Tiến)
            float cdMultiplier = 1f - GetStack(BlessingEffectType.DashCooldown) * 0.1f;
            if (IsFrenzyActive) cdMultiplier -= 0.18f;
            playerController.dashCooldown = Mathf.Max(0.12f, baseDashCooldown * Mathf.Clamp(cdMultiplier, 0.35f, 1f));
        }

        // Tốc độ đánh (Cờ Khởi Nghĩa + Xuân Kỷ Dậu)
        if (playerCombat != null)
        {
            float atkSpeed = 1f + GetStack(BlessingEffectType.AttackSpeed) * 0.11f;
            if (IsFrenzyActive) atkSpeed += 0.22f;
            playerCombat.attackCooldown = Mathf.Max(0.16f, baseAttackCooldown / atkSpeed);
        }

        // Giảm sát thương nhận (Thành Giáp Âu Lạc + Thành Cổ Loa passive)
        if (playerHealth != null)
        {
            float reduction = GetStack(BlessingEffectType.Armor)        * 0.075f
                            + GetStack(BlessingEffectType.CoLoaCitadel) * 0.045f;
            playerHealth.DamageReduction = Mathf.Clamp(reduction, 0f, 0.65f);
        }
    }

    // ──────────────────────────────────────────────────────────────────
    //  Private – Dash Blessing Handlers
    // ──────────────────────────────────────────────────────────────────
    private void BeginDashBlessings()
    {
        ...
        // Tường Thành – tạo kết giới
        int barrierStack = GetStack(BlessingEffectType.DashBarrier);
        if (barrierStack > 0) CreateBarrier(transform.position, barrierStack);

        // Bóng Chiến Trường – tạo phân thân
        int decoyStack = GetStack(BlessingEffectType.DashDecoy);
        if (decoyStack > 0) CreateDecoy(transform.position, decoyStack);

        TickDashDamage(force: true);
    }
    ...
    private void TickDashDamage(bool force = false)
    {
        ...
        if (dashStack <= 0 && elephantStack <= 0) return;

        dashHitTimer += Time.deltaTime;
        if (!force && dashHitTimer < 0.045f) return;
        dashHitTimer = 0f;

        float radius = elephantStack > 0 ? 2.05f : 1.35f;
        int damage   = 12 * dashStack + 34 * elephantStack;
        int hitCount = Physics.OverlapSphereNonAlloc(
            transform.position + Vector3.up * 0.75f, radius,
            overlapHits, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (enemy == null || enemy.IsDead || dashHitTargets.Contains(enemy)) continue;
            ...
            SpawnPulse(enemy.transform.position + Vector3.up * 0.75f, mobilityColor,
                elephantStack > 0 ? 2f : 1.15f, 0.32f, "Blessing_DashImpact");
    ...
    {
        // Truy Kích: đánh dấu đòn đầu tiên sau Dash sẽ được buff
        if (GetStack(BlessingEffectType.PostDashDamage) > 0)
            postDashReady = true;
    }

    // ──────────────────────────────────────────────────────────────────
    //  Private – Thành Cổ Loa (Shield ticking)
    // ──────────────────────────────────────────────────────────────────
    ...
    // ──────────────────────────────────────────────────────────────────
    //  Private – Area Damage + Enemy Queries
    // ──────────────────────────────────────────────────────────────────
    private void ApplyAreaDamage(Vector3 center, float radius, int damage, Color color, string effectName)
    {
        int hitCount = Physics.OverlapSphereNonAlloc(center, radius, overlapHits,
    ...
    private void CollectNearestEnemies(Vector3 center, float radius,
        List<MinionHealth3D> results, IReadOnlyCollection<MinionHealth3D> excluded)
    {
        results.Clear();
        int hitCount = Physics.OverlapSphereNonAlloc(center, radius, overlapHits,
            Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            MinionHealth3D enemy = hit.GetComponentInParent<MinionHealth3D>();
            if (enemy == null || enemy.IsDead || results.Contains(enemy)) continue;
            if (ContainsTarget(excluded, enemy)) continue;
            results.Add(enemy);
        }
    ...
    // ──────────────────────────────────────────────────────────────────
    //  Private – Dash Blessing Visuals (Barrier + Decoy)
    // ──────────────────────────────────────────────────────────────────
    private void CreateBarrier(Vector3 position, int stack)
    {
        ...
        if (rend != null)
            rend.sharedMaterial = CreateRuntimeMaterial("Blessing_TuongThanh_Mat", defenseColor, defenseColor * 1.3f);
    ...
    private void CreateDecoy(Vector3 position, int stack)
    {
        ...
        Collider col = decoy.GetComponent<Collider>();
        if (col != null) Destroy(col);

        Renderer rend = decoy.GetComponent<Renderer>();
        if (rend != null)
            rend.sharedMaterial = CreateRuntimeMaterial("Blessing_Decoy_Mat", mobilityColor, mobilityColor * 1.5f);
    ...
    private void SpawnPulse(Vector3 position, Color color, float size, float lifetime, string objectName)
    {
        ...
        Renderer rend = pulse.GetComponent<Renderer>();
        if (rend != null)
        {
            rend.sharedMaterial          = CreateRuntimeMaterial(objectName + "_Mat", color, color * 1.8f);
    ...
    private void SpawnLine(Vector3 start, Vector3 end, Color color, string objectName)
    {
        ...
        if (rend != null)
        {
            rend.sharedMaterial    = CreateRuntimeMaterial(objectName + "_Mat", color, color * 2f);
            rend.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
            rend.receiveShadows    = false;
        }
    ...
    private static Material CreateRuntimeMaterial(string matName, Color color, Color emission)
    {
